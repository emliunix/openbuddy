// AUTO-PORTED from claude-desktop-buddy/src/buddies/dragon.cpp
...
static void doSleep(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xF800);
  ...
  buddy_set_cursor(BUDDY_X_CENTER + 22 + p1, BUDDY_Y_OVERLAY + 18 - p1 * 2);
  buddy_print("z");
  buddy_set_color(BUDDY_WHITE);
  buddy_set_cursor(BUDDY_X_CENTER + 28 + p2, BUDDY_Y_OVERLAY + 12 - p2);
  buddy_print("Z");
  // Smoke ring rising
  buddy_set_color(BUDDY_DIM);
  buddy_set_cursor(BUDDY_X_CENTER + 18, BUDDY_Y_OVERLAY + 16 - p3);
  buddy_print("o");
}
...
static void doIdle(uint32_t t) {
  ...
  if (pose == 6 || pose == 7) {
    int s = (t & 7);
    buddy_set_color(BUDDY_DIM);
    buddy_set_cursor(BUDDY_X_CENTER + (pose == 7 ? 14 : 0), BUDDY_Y_OVERLAY - 4 - s);
    buddy_print(s & 1 ? "o" : ".");
  }
}
...
static void doBusy(uint32_t t) {
  ...
  buddy_print(COINS[t % 6]);
  // Sparkle on top of pile
  if ((t / 2) & 1) {
    buddy_set_color(BUDDY_WHITE);
    buddy_set_cursor(BUDDY_X_CENTER + 24, BUDDY_Y_OVERLAY + 10);
    buddy_print("*");
  }
}
...
static void doAttention(uint32_t t) {
  ...
  buddy_print_sprite(P[pose], 5, 0, 0xF800, xOff);
  ...
  if (pose == 3 || pose == 4) {
    buddy_set_color(BUDDY_YEL);
    buddy_set_cursor(BUDDY_X_CENTER + 18, BUDDY_Y_OVERLAY + 12 - ((t * 2) % 8));
    buddy_print("^");
  }
}
...
static void doCelebrate(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0xF800);
...
static void doDizzy(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xF800, X_SHIFT[beat]);
...
static void doHeart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  buddy_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0xF800);
  ...
  int sp = (t * 2) % 18;
  buddy_set_color(BUDDY_DIM);
  buddy_set_cursor(BUDDY_X_CENTER + 14, BUDDY_Y_OVERLAY + 14 - sp);
  buddy_print("o");
}
