// AUTO-PORTED from claude-desktop-buddy/src/buddies/snail.cpp
...
static void doSleep(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xD8FE);
  ...
  buddy_set_cursor(BUDDY_X_CENTER + 8 - p1, BUDDY_Y_OVERLAY + 14 - p1);
  buddy_print("z");
  buddy_set_color(BUDDY_WHITE);
  buddy_set_cursor(BUDDY_X_CENTER + 14 - p2, BUDDY_Y_OVERLAY + 10 - p2);
  buddy_print("Z");
  buddy_set_color(BUDDY_DIM);
  buddy_set_cursor(BUDDY_X_CENTER + 4 - p3 / 2, BUDDY_Y_OVERLAY + 6 - p3 / 2);
  buddy_print("z");
}
...
static void doIdle(uint32_t t) {
  ...
  if ((t / 6) & 1) {
    ...
    buddy_set_cursor(BUDDY_X_CENTER - 10, BUDDY_Y_BASE + 32);
    buddy_print(".");
  }
}
...
static void doAttention(uint32_t t) {
  ...
  buddy_print_sprite(P[pose], 5, 0, 0xD8FE, xOff);
...
static void doCelebrate(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0xD8FE);
...
static void doDizzy(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xD8FE, X_SHIFT[beat]);
...
static void doHeart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  buddy_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0xD8FE);
