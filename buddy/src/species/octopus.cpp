// AUTO-PORTED from claude-desktop-buddy/src/buddies/octopus.cpp
...
static void doSleep(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xA01F);
  ...
  buddy_set_cursor(BUDDY_X_CENTER + 18 + p1, BUDDY_Y_OVERLAY + 16 - p1 * 2);
  buddy_print("z");
  buddy_set_color(BUDDY_WHITE);
  buddy_set_cursor(BUDDY_X_CENTER + 24 + p2, BUDDY_Y_OVERLAY + 12 - p2);
  buddy_print("Z");
  buddy_set_color(BUDDY_DIM);
  buddy_set_cursor(BUDDY_X_CENTER + 14 + p3 / 2, BUDDY_Y_OVERLAY + 8 - p3 / 2);
  buddy_print("z");
}
...
static void doBusy(uint32_t t) {
  ...
  int b = (t * 2) % 18;
  buddy_set_color(BUDDY_WHITE);
  buddy_set_cursor(BUDDY_X_CENTER - 30, BUDDY_Y_OVERLAY + 18 - b);
  buddy_print("o");
}
...
static void doAttention(uint32_t t) {
  ...
  buddy_print_sprite(P[pose], 5, 0, 0xA01F, xOff);
...
static void doCelebrate(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0xA01F);
...
static void doDizzy(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xA01F, X_SHIFT[beat]);
  ...
  uint8_t p2 = (t + 4) % 8;
  buddy_set_color(BUDDY_CYAN);
  buddy_set_cursor(BUDDY_X_CENTER + OX[p1] - 2, BUDDY_Y_OVERLAY + 4 + OY[p1]);
  buddy_print("*");
  buddy_set_color(BUDDY_PURPLE);
  buddy_set_cursor(BUDDY_X_CENTER + OX[p2] - 2, BUDDY_Y_OVERLAY + 4 + OY[p2]);
  buddy_print("*");
  ...
  if ((t / 8) & 1) {
    int puff = (t % 8);
    buddy_set_color(BUDDY_DIM);
    buddy_set_cursor(BUDDY_X_CENTER - 24 - puff, BUDDY_Y_OVERLAY + 10 + puff);
    buddy_print("o");
    buddy_set_cursor(BUDDY_X_CENTER + 24 + puff, BUDDY_Y_OVERLAY + 10 + puff);
    buddy_print("o");
  }
...
static void doHeart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  buddy_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0xA01F);
