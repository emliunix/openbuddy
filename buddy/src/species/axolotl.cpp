// AUTO-PORTED from claude-desktop-buddy/src/buddies/axolotl.cpp
...
static void doSleep(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xFB1E);
  ...
  buddy_set_cursor(BUDDY_X_CENTER + 18 + (p1 % 3), BUDDY_Y_OVERLAY + 20 - p1 * 2);
  buddy_print("o");
  buddy_set_color(BUDDY_WHITE);
  buddy_set_cursor(BUDDY_X_CENTER + 26 - (p2 % 4), BUDDY_Y_OVERLAY + 18 - p2);
  buddy_print("O");
  buddy_set_color(BUDDY_DIM);
  buddy_set_cursor(BUDDY_X_CENTER + 14 + (p3 % 5), BUDDY_Y_OVERLAY + 12 - p3 / 2);
  buddy_print("z");
}
...
static void doIdle(uint32_t t) {
  ...
  int p = (t / 2) % 14;
  buddy_set_color(BUDDY_CYAN);
  buddy_set_cursor(BUDDY_X_CENTER + 24, BUDDY_Y_OVERLAY + 16 - p);
  buddy_print(p & 1 ? "o" : "O");
}
...
static void doBusy(uint32_t t) {
  ...
  int b = (t * 2) % 10;
  buddy_set_color(BUDDY_CYAN);
  buddy_set_cursor(BUDDY_X_CENTER - 28, BUDDY_Y_OVERLAY + 18 - b);
  buddy_print("o");
}
...
static void doAttention(uint32_t t) {
  ...
  buddy_print_sprite(P[pose], 5, 0, 0xFB1E, xOff);

  if ((t / 2) & 1) {
    buddy_set_color(BUDDY_YEL);
    buddy_set_cursor(BUDDY_X_CENTER - 4, BUDDY_Y_OVERLAY);
    buddy_print("!");
  }
  if ((t / 3) & 1) {
    buddy_set_color(BUDDY_RED);
    buddy_set_cursor(BUDDY_X_CENTER + 4, BUDDY_Y_OVERLAY + 4);
    buddy_print("!");
  }
}
...
static void doCelebrate(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, Y_SHIFT[beat], 0xFB1E);
...
static void doDizzy(uint32_t t) {
  ...
  buddy_print_sprite(P[SEQ[beat]], 5, 0, 0xFB1E, X_SHIFT[beat]);
...
static void doHeart(uint32_t t) {
  ...
  uint8_t beat = (t / 5) % sizeof(SEQ);
  buddy_print_sprite(P[SEQ[beat]], 5, Y_BOB[beat], 0xFB1E);
  ...
  int b = (t + 3) % 14;
  buddy_set_color(BUDDY_HEART);
  buddy_set_cursor(BUDDY_X_CENTER + 26, BUDDY_Y_OVERLAY + 16 - b);
  buddy_print(b & 1 ? "o" : "O");
}
